...
public class HostControllerBootOperationsTestCase {
    ...
    private static List<String> serverThreeChildrenTypes;
    ...
    public static void setupDomain() throws Exception {
        ...
        final DomainTestSupport.Configuration configuration = DomainTestSupport.Configuration.create(OperationTimeoutTestCase.class.getSimpleName(),
                ...
                false,
                false,
                Boolean.getBoolean("wildfly.master.debug"),
                false,
                Boolean.getBoolean("wildfly.slave.debug")
        );
        ...
        op = Util.createEmptyOperation(READ_CHILDREN_TYPES_OPERATION, SLAVE_ADDR.append(SERVER_CONFIG_MAIN_THREE));
        result = DomainTestUtils.executeForResult(op, masterClient);
        serverThreeChildrenTypes = result.asList().stream().map(m -> m.asString()).collect(Collectors.toList());
    ...
    public void testManagementOperationsWhenSlaveHCisBooting() throws Exception {
        ...
        // The DC write lock is acquired at this time because the servers are starting, although they have not been registered in the domain yet
        // Read operations should pass, even those that use proxies
        ...
        // at this point the server-three is still rejecting operations, that means a read using proxies=true could fail for reading this server
...
//        op = Util.createEmptyOperation(READ_RESOURCE_OPERATION, SLAVE_ADDR.append(CORE_SERVICE_PATCHING));
//        addCommonReadOperationAttributes(op, skipServerDirectReads);
//        DomainTestUtils.executeForResult(op, masterClient);


        // verify reads in a composite operations
        // this will test the code path with the changes implemented in XXXXXXXXXX
        // assert we are able to execute a read operations targeted to a single host in a composite when the write lock is already acquired
