...
public class ServerInventoryImpl implements ServerInventory {
    ...
    @Override
    public boolean reconnectServer(final String serverName, final ModelNode domainModel, final String authKey, final boolean running, final boolean stopping, final boolean blockUntilStopped, boolean requestRestart, boolean restartBlocking) {
        if(shutdown || connectionFinished) {
            throw HostControllerLogger.ROOT_LOGGER.hostAlreadyShutdown();
        }
