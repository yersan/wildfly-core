...
public abstract class DomainControllerWriteAttributeHandler extends ReloadRequiredWriteAttributeHandler {
    ...
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        ...
        if (operation.hasDefined(VALUE, LOCAL)) {
            ...
            if (remoteDC.hasDefined(ADMIN_ONLY_POLICY.getName()) && remoteDC.get(ADMIN_ONLY_POLICY.getName()).asString().equals(AdminOnlyDomainConfigPolicy.LEGACY_FETCH_FROM_DOMAIN_CONTROLLER.toString())) {
                ControllerLogger.ROOT_LOGGER.info("\"fetch-from-master\" is a deprecated value for \"domain-controller.remote.admin-only-policy\", \"fetch-from-domain-controller\" will be used instead.");
